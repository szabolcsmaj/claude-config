...
- If every sub-agent is finished, present the findings to the user by categorizing the type of errors
- Focus on urgent or critical issues first.
- Save the findings into <findings_file>. If no <findings_file> was specified, create a new one with the name of `audit_findings.md`. If this file already exists, create a new one with a suffix of `_1` (e.g.: `audit_findings_1.md`, etc)
- The <findings_file> should group the different issues under topics, then under each topic, it should list all related issues by number, state the question, then state a decision under it. The decision part should have the value of "Unanswered"
