# 5. Clarify the issues in the audit findings, then update specs/*.md files
...
- Ask the user if:
   - Whether he wants to verify the updates were applied correctly with `/ralph:5-ensure-answers-were-moved-to-spec <findings_file>`?
   - Whether he wants to rerun the audit on the spec files again with the `/ralph:3-audit-specs` command?
   - Or if he wants to start creating the implementation plan with `run-ralph prompt_plan.md` or if `run-ralph` command is not set, use `while true; cat prompt_plan.md | cldy -p --output-format=stream-json --model opus --verbose; end`
