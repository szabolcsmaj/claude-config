...
- `$ARGUMENTS` - (optional) Path to the product requirements document. Defaults to `prd.md` if not provided.

## Instructions

1. Study the PRD file (use `$ARGUMENTS` if provided, otherwise use `prd.md`) 
...
7. Create a CLAUDE.md file and add all the technical commands which are needed to develop on the application: running linters, formatters, unit tests, integration tests, type checker, scripts for backup/start/restore/etc
